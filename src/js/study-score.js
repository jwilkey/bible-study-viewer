...
const judge = (value) => {
  ...
  const goodObject = () =>
    Object.keys(value).length > 0 && Object.values(value).every(judge)
...
const template = {
  ...
  interpret: {
    ...
    keywords: ['array of strings'] || {
      keyword: 'argument for why it is a keyword'
    },
  ...
  application: {
    ...
    ACTS: {
      ...
      confession: [
        'questions derived from the text which facilitate confession of sin'
      ],
      thanks: ['questions derived from the text which foster thankfulness'],
      supplication: [
        'questions derived from the text which inform requests to God'
      ]
    },
    integrity: [
      { question: '', will: 0, intellect: 0, fantasy: 0, comment: '' }
    ]
  }
}
